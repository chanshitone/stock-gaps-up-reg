"""
Peak-capital calculator with index-based initial sizing and fixed add-on logic.
...
This v3 variant changes only the initial allocation rule:
    ...
    - add-on logic is unchanged from v2
...
Usage:
    python run_peak_capital_v3.py --trades outputs/<run>/trades.csv
    python run_peak_capital_v3.py --trades outputs/<run>/trades.csv --per-trade 20000
    python run_peak_capital_v3.py --trades outputs/<run>/trades.csv --per-trade 15000 --add-on-per-trade 20000
    python run_peak_capital_v3.py --trades outputs/<run>/trades.csv --initial-principal 132470
    python run_peak_capital_v3.py --trades outputs/<run>/trades.csv --config config/strategy.yaml
    python run_peak_capital_v3.py --trades outputs/<run>/trades.csv --add-on-csv outputs/<run>/add_on_orders.csv
    python run_peak_capital_v3.py --trades outputs/<run>/trades.csv --daily-win-loss-csv outputs/<run>/daily_win_loss.csv
"""
...
def _load_traded_rows(trades_path: Path) -> pd.DataFrame:
    ...
    if DEFAULT_INDEX_COLUMN not in df.columns:
        raise ValueError(f"{trades_path} is missing required column: {DEFAULT_INDEX_COLUMN}")
...
def _build_trade_timeline(
...
) -> tuple[list[tuple], list[dict[str, object]], list[dict[str, object]]]:
    events: list[tuple] = []
    ...
    return events, add_on_orders, position_legs
...
def run(
    ...
    events, add_on_orders, position_legs = _build_trade_timeline(traded, client, resolved_add_on_per_trade)

    if not events:
        print("No initial positions were opened after applying the index sizing rule.")
        return
    ...
    executed_trades = traded[traded["shares"] > 0].copy()
    ...
    skipped_trades = int((traded["shares"] <= 0).sum())

    print(f"\n{'='*70}")
    print("  Peak Capital Calculator V3  (index-sized initial buys + fixed add-on buys)")
    ...
    print(f"  Skipped buys  : {skipped_trades}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calculate minimum principal with index-sized initial buys and a >5-day 1R add-on rule."
    )
